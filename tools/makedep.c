...
#define NINJA_HASH_SIZE 4093
#define NINJA_DOLLAR 36
...
static struct list ninja_hash[NINJA_HASH_SIZE];

static unsigned int hash_ninja_target( const char *name )
{
    unsigned int ret = 2166136261u;
    while (*name) ret = (ret * 16777619) ^ *name++;
...
static void parse_ninja_makefile( const char *name )
{
    struct strarray current_targets = empty_strarray;
    FILE *file;
    char *line;
    unsigned int i;

    for (i = 0; i < NINJA_HASH_SIZE; i++) list_init( &ninja_hash[i] );

    if (!(file = fopen( name, "r" ))) fatal_error( "cannot open %s\n", name );
    input_line = 0;
    ...
    while ((line = get_line( file )))
    {
        ...
        {
            p++;
            STRARRAY_FOR_EACH( target_name, &current_targets )
                strarray_add( &get_ninja_target( target_name )->commands, xstrdup( p ));
            continue;
        }

        current_targets.count = 0;
        p = skip_spaces( p );
        if (!*p || *p == '#' || is_make_assignment( p )) continue;
        if (!(colon = strchr( p, ':' ))) continue;

        *colon = 0;
        deps = colon + (colon[1] == ':' ? 2 : 1);
        targets = p;

        if (!strcmp( targets, ".PHONY" ))
        {
        ...
        if (!strcmp( targets, ".INIT" ) || !strcmp( targets, ".PRECIOUS" ) ||
            !strcmp( targets, ".MAKEFILEDEPS" ) || !strcmp( targets, ".SUFFIXES" ))
            continue;
    ...
    if (fclose( file )) fatal_perror( "close" );
    input_file_name = NULL;
}
...
static void ninja_buffer_addn( struct ninja_buffer *buffer, const char *str, size_t len )
{
    size_t new_size;

    if (buffer->len + len + 1 > buffer->size)
    {
        new_size = buffer->size ? buffer->size * 2 : 128;
        if (new_size < buffer->len + len + 1) new_size = buffer->len + len + 64;
        buffer->size = new_size;
        buffer->str = xrealloc( buffer->str, buffer->size );
    }
...
static void ninja_buffer_add( struct ninja_buffer *buffer, const char *str )
{
    ninja_buffer_addn( buffer, str, strlen(str) );
}

static void ninja_buffer_add_char( struct ninja_buffer *buffer, char ch )
{
    ninja_buffer_addn( buffer, &ch, 1 );
}

static void ninja_buffer_add_dollars( struct ninja_buffer *buffer, unsigned int count )
{
    while (count--) ninja_buffer_add_char( buffer, NINJA_DOLLAR );
}

static void ninja_buffer_add_shell_text( struct ninja_buffer *buffer, const char *str )
{
    while (*str)
    {
        if (*str == NINJA_DOLLAR)
        {
            ...
            if (str[1] == NINJA_DOLLAR) str += 2;
            else str++;
        }
        else ninja_buffer_add_char( buffer, *str++ );
    }
}
...
static char *expand_ninja_command( const char *command, const char *target )
{
    ...
    if (p[0] == NINJA_DOLLAR && p[1] == '(' && !strncmp( p + 2, "quiet_", 6 ))
    {
    ...
    while (*p)
    {
        ...
        {
            ninja_buffer_add_dollars( &buffer, 2 );
            if (p[1] == NINJA_DOLLAR) p += 2;
            else p++;
            continue;
        }

        ninja_buffer_add_char( &buffer, *p++ );
    }
...
static void output_ninja_path( const char *name )
{
    for (; *name; name++)
    {
        ...
        {
        case NINJA_DOLLAR:
            output( "%c%c", NINJA_DOLLAR, NINJA_DOLLAR );
            break;
        case ' ':
            output( "%c ", NINJA_DOLLAR );
            break;
        case ':':
            output( "%c:", NINJA_DOLLAR );
            break;
...
static void output_ninja_file( const char *makefile_name )
{
    ...
    output( "  command = %ccmd\n", NINJA_DOLLAR );
    output( "  description = BUILD %cout\n", NINJA_DOLLAR );
    output( "  restat = 1\n\n" );
    output( "rule wine_generator\n" );
    output( "  command = %ccmd\n", NINJA_DOLLAR );
    output( "  description = REGEN %cout\n", NINJA_DOLLAR );
    output( "  generator = 1\n" );
    output( "  restat = 1\n\n" );
