...
static const char *debugstr_fmtex(const WAVEFORMATEX *fmt)
{
    ...
    else if(fmt->wFormatTag == WAVE_FORMAT_EXTENSIBLE)
    {
        ...
        snprintf(buf, sizeof(buf), "tag: 0x%x (%s), ch: %u (mask: 0x%lx), rate: %lu, depth: %u",
                (unsigned int)fmt->wFormatTag, debugstr_guid(&fmtex->SubFormat),
                (unsigned int)fmt->nChannels, (unsigned long)fmtex->dwChannelMask,
                (unsigned long)fmt->nSamplesPerSec, (unsigned int)fmt->wBitsPerSample);
    }
    else
    {
