...
static D3DXVECTOR4 * d3dx_vec4_transform(D3DXVECTOR4 *out, const D3DXVECTOR4 *v, const D3DXMATRIX *m)
{
...
static D3DXVECTOR4 * d3dx_vec4_transform_array(D3DXVECTOR4 *out, UINT out_stride,
        const D3DXVECTOR4 *v, UINT in_stride, const D3DXMATRIX *m, UINT count)
{
    for (unsigned int i = 0; i < count; ++i)
    {
        d3dx_vec4_transform((D3DXVECTOR4 *)((char *)out + out_stride * i),
                (const D3DXVECTOR4 *)((const char *)v + in_stride * i), m);
    }

    return out;
}

static D3DXMATRIX * d3dx_matrix_multiply(D3DXMATRIX *out, const D3DXMATRIX *m1, const D3DXMATRIX *m2)
{
    int i, j;
    ...
    for (i = 0; i < 4; ++i)
    {
        for (j = 0; j < 4; ++j)
        {
            out->m[i][j] = m1->m[i][0] * m2->m[0][j] + m1->m[i][1] * m2->m[1][j] +
...
static D3DX10_SPRITE * d3dx10_get_next_sprite(D3DX10_SPRITE *ptr, unsigned int stride)
{
    return (D3DX10_SPRITE *)((char *)ptr + stride);
}

static void d3dx10_sprite_draw_batch(struct d3dx10_sprite *sprite, D3DX10_SPRITE **sprites,
        unsigned int count, unsigned int stride)
{
    ...
    D3DX10_SPRITE *ptr = *sprites, *start_sprite;
    ...
    for (i = 0; i < count; ++i, v += 4)
    {
        ...
        v[1] = v[2] = v[3] = *v;
        d3dx_vec4_transform_array(&v->pos, sizeof(*v), quad, sizeof(*quad), &ptr->matWorld, 4);
    ...
    for (i = 0; i < count; ++i)
    {
        if (ptr->pTexture != start_sprite->pTexture || i == count - 1)
        {
            ...
            ID3D10Device_Draw(sprite->device, (i - start + 1) * 4, start * 4);
            start_sprite = ptr;
            start = i;
        }
    ...
    *sprites = (D3DX10_SPRITE *)((char *)*sprites + count * stride);
}
        ...
        size_t count, unsigned int stride)
{
    ...
    ID3D10Device_IASetPrimitiveTopology(sprite->device, D3D10_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
    ...
    d3dx_matrix_multiply(&m, &sprite->projection, &sprite->view);
    ID3D10Device_UpdateSubresource(sprite->device, (ID3D10Resource *)sprite->vs_cb, 0, NULL,
            &m, 0, 0);

    for (i = 0; i < count / sprite->batch_size; ++i)
        d3dx10_sprite_draw_batch(sprite, &ptr, sprite->batch_size, stride);

    d3dx10_sprite_draw_batch(sprite, &ptr, count % sprite->batch_size, stride);
}
