...
/* Global */
static struct game_input *gameinput = NULL;
...
static HRESULT WINAPI game_input_QueryInterface( v0_IGameInput *iface, REFIID iid, void **out )
{
    ...
    *out = NULL;
    ...
    else if (IsEqualGUID( iid, &IID_v1_IGameInput ))
        *out = &impl->v1_IGameInput_iface;
    else if (IsEqualGUID( iid, &IID_v2_IGameInput ))
...
{
    FIXME( "iface %p GetCurrentReading kind=%d device=%p reading=%p\n", iface, kind, device, reading );
...
{
    FIXME( "iface %p GetNextReading reference=%p kind=%d device=%p reading=%p\n", iface, reference, kind, device, reading );
    if (reading) *reading = NULL;
    return E_NOTIMPL;
}
...
static HRESULT WINAPI game_input_GetPreviousReading( v0_IGameInput *iface, v0_IGameInputReading *reference,
                                                     ...
                                                     v0_IGameInputReading **reading )
{
    FIXME( "iface %p GetPreviousReading reference=%p kind=%d device=%p reading=%p\n", iface, reference, kind, device, reading );
    if (reading) *reading = NULL;
    return E_NOTIMPL;
}
...
static HRESULT WINAPI game_input_GetTemporalReading( v0_IGameInput *iface, uint64_t timestamp,
                                                    v0_IGameInputDevice *device, v0_IGameInputReading **reading )
{
    FIXME( "iface %p GetTemporalReading timestamp=%llu device=%p reading=%p\n", iface, timestamp, device, reading );
    if (reading) *reading = NULL;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p RegisterReadingCallback device=%p kind=%d threshold=%f callback=%p token=%p\n",
           iface, device, kind, threshold, callback, token );
...
{
    FIXME( "iface %p RegisterDeviceCallback device=%p kind=%d filter=%d enum_kind=%d callback=%p token=%p\n",
...
{
    FIXME( "iface %p RegisterSystemButtonCallback device=%p filter=%u callback=%p token=%p\n",
           iface, device, filter, callback, token );
    if (token) *token = (GameInputCallbackToken)0;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p RegisterKeyboardLayoutCallback device=%p callback=%p token=%p\n",
...
{
    FIXME( "iface %p CreateAggregateDevice kind=%d device=%p\n", iface, kind, device );
...
{
    FIXME( "iface %p FindDeviceFromId value=%p device=%p\n", iface, value, device );
    if (device) *device = NULL;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p FindDeviceFromObject unknown=%p device=%p\n", iface, value, device );
    if (device) *device = NULL;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p FindDeviceFromPlatformHandle handle=%p device=%p\n", iface, value, device );
    if (device) *device = NULL;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p FindDeviceFromPlatformString value=%p device=%p\n", iface, value, device );
    if (device) *device = NULL;
    return E_NOTIMPL;
}
                                                         ...
                                                         uint8_t iface_num, uint8_t collection )
{
    FIXME( "iface %p EnableOemDeviceSupport vid=0x%04x pid=0x%04x iface=%u collection=%u\n",
           iface, vid, pid, iface_num, collection );
    return E_NOTIMPL;
}
...
static HRESULT WINAPI game_input1_QueryInterface( v1_IGameInput *iface, REFIID iid, void **out )
{
    ...
    return v0_IGameInput_QueryInterface( &impl->v0_IGameInput_iface, iid, out );
}

static ULONG WINAPI game_input1_AddRef( v1_IGameInput *iface )
{
    struct game_input *impl = impl_from_v1_IGameInput( iface );
    return v0_IGameInput_AddRef( &impl->v0_IGameInput_iface);
}

static ULONG WINAPI game_input1_Release( v1_IGameInput *iface )
{
    struct game_input *impl = impl_from_v1_IGameInput( iface );
    return v0_IGameInput_Release( &impl->v0_IGameInput_iface);
};
...
{
    FIXME( "iface %p GetCurrentReading kind=%d device=%p =%p\n", iface, kind, device, reading );
    if (reading) *reading = NULL;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p RegisterReadingCallback device=%p kind=%d callback=%p token=%p\n",
           iface, device, kind, callback, token );
    if (token) *token = (GameInputCallbackToken)0;
    return E_NOTIMPL;
}
...
{
    FIXME( "iface %p kind %d device %p reading %p\n", iface, kind, device, reading );
...
HRESULT WINAPI GameInputCreate( v0_IGameInput **out )
{
    ...
    if (gameinput)
    {
        v0_IGameInput_AddRef( &gameinput->v0_IGameInput_iface );
        *out = &gameinput->v0_IGameInput_iface;
        return S_OK;
    }

    if (!(gameinput = calloc( 1, sizeof(*gameinput) )))
        return E_OUTOFMEMORY;

    gameinput->v0_IGameInput_iface.lpVtbl = &game_input_vtbl;
    gameinput->v1_IGameInput_iface.lpVtbl = &game_input1_vtbl;
    gameinput->v2_IGameInput_iface.lpVtbl = &game_input2_vtbl;
    gameinput->ref = 1;
