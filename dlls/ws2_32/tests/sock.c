...
static void test_base_handle(void)
{
    ...
    static const DWORD ioctl_codes[] =
    {
        SIO_BASE_HANDLE,
    ...
    for (i = 0; i < ARRAY_SIZE(tests); ++i)
    {
        for (j = 0; j < ARRAY_SIZE(ioctl_codes); ++j)
        {
            ...
            WSASetLastError(0xdeadbeef);
            ret = WSAIoctl(s, ioctl_codes[j], NULL, 0, &base, sizeof(base), NULL, &overlapped, NULL);
            ok(ret == -1, "expected failure\n");
            ok(WSAGetLastError() == WSAEFAULT, "got error %u\n", WSAGetLastError());
            ...
            size = 0xdeadbeef;
            base = 0xdeadbeef;
            ret = WSAIoctl(s, ioctl_codes[j], NULL, 0, &base, sizeof(base), &size, NULL, NULL);
            ok(!ret, "expected success\n");
            ok(!WSAGetLastError(), "got error %u\n", WSAGetLastError());
            ...
            overlapped.Internal = 0xdeadbeef;
            overlapped.InternalHigh = 0xdeadbeef;
            ret = WSAIoctl(s, ioctl_codes[j], NULL, 0, &base, sizeof(base), &size, &overlapped, NULL);
            ok(ret == -1, "expected failure\n");
            ok(WSAGetLastError() == ERROR_IO_PENDING, "got error %u\n", WSAGetLastError());
            ok(size == 0xdeadbeef, "got size %lu\n", size);
            ...
            ret = WSAIoctl(s, ioctl_codes[j], NULL, 0, &base, sizeof(base), NULL, &overlapped, socket_apc);
            ok(ret == -1, "expected failure\n");
            ok(WSAGetLastError() == WSAEFAULT, "got error %u\n", WSAGetLastError());
            ...
            size = 0xdeadbeef;
            base = 0xdeadbeef;
            ret = WSAIoctl(s, ioctl_codes[j], NULL, 0, &base, sizeof(base), &size, &overlapped, socket_apc);
            ok(ret == -1, "expected failure\n");
            ok(WSAGetLastError() == ERROR_IO_PENDING, "got error %u\n", WSAGetLastError());
            ok(size == 0xdeadbeef, "got size %lu\n", size);
